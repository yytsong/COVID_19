...
# c <-c("China", "Australia", "US", "Russia", "UK", "South Korea")
...
plot_explore_country <- function(df, xc, xm, xa, yc, ym, ya, min_case){
  ...
  df %>% 
    ...
    labs (caption = str_c("\n","By: @behrooz_hm @yurisyt (Monash University) / Data: Tim Churches (UNSW) & Johns Hopkins University"),
          x = str_c(xm, xc, xa, "Log Scale",sep = " - "),
          title = str_c ("COVID-19 - Since Number of Confirmed Cases >= ",min_case," @ ", Sys.time()," AEDT", "\n","(XY-axis are logged)"),
...
plot_world_lvl <- function(lvl){
  ...
  df %>% 
    ...
    labs(x = "Days since the First Case Reported from each Country", 
         y = str_c("Confirmed Cases Per Capita"),
         title = str_c ("COVID-19 Cases by ", lvl, " @ ", Sys.time()," AEDT", "\n","(Y-axis is logged)"),
         caption = str_c("\n","By: @behrooz_hm @yurisyt (Monash University) / Data: Tim Churches (UNSW) & Johns Hopkins University"))
 ...
 if(g[1] == "world"){
  ...
  df_new %>% 
    ...
    labs (
      ...
      caption = str_c("\n","By: @behrooz_hm @yurisyt (Monash University) / Data: Tim Churches (UNSW) & Johns Hopkins University"),
      x = str_c("Days since ", m, " >= ", min_case, " from each ", xtitle, sep = ""),
      title = str_c ("Per Capita COVID-19 ", a, " @ ", Sys.time()," AEDT", "\n","(Y-axis is logged)"),
  ...
  df_new %>% 
   ...
   # geom_abline(slope = 0.75, linetype = "dashed", alpha = 0.5, intercept = -0.1)+
    ...
    labs (caption = str_c("\n","By: @behrooz_hm @yurisyt (Monash University) / Data: Tim Churches (UNSW) & Johns Hopkins University"),
          ...
          title = str_c ("COVID-19 ", a, " (since ", min_case ," Cases) Growth Rate @ ", Sys.time()," AEDT", "\n",lm_eqn(df = df_new), "(XY-axis is logged)"),
...
# 
# 
# names(c25) <- ch_state_sorted_by_cases 
# ch_state_color <- c25[1:length(ch_state_sorted_by_cases)]
