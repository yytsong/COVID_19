### this script is to explore EpiModel
### source https://rviews.rstudio.com/2020/03/19/simulating-covid-19-interventions-with-r/?mkt_tok=eyJpIjoiTW1Zd1pXUTVaVEJpTmpabCIsInQiOiJJMDdjZE5mSU1hRlNwdkkwRktCT0Y5dTZmOTMybElQXC9pcWRvU29ZYTR4NTFaVEE4eUN3Z0ZDTlhtYTZaelJQaE84SGd4a0hIa1Q0ajc0M1wvNkxqTURvWDBFMVwvMkJcL244N0JoYTc0UWFpWXFiMnNHUWYxVnBYbkF5QWdSb3c2cFYifQ%3D%3D
### source https://timchurches.github.io/blog/posts/2020-03-10-modelling-the-effects-of-public-health-interventions-on-covid-19-transmission-part-1/

# Load EpiModel
suppressMessages(c(library(EpiModel), library(tidyverse), library(incidence), library(earlyR)))


# The step time unit per se doesn’t need to be explicitly specified, but obviously the time unit affects 
# the number of time steps to be simulated, via the nsteps parameter. 
...
plot(sim)
plot(sim, y = "si.flow", mean.col = "red", qnts.col = "red") # si.flow is the incidence (count of new cases per day)
...
run_sir_sim <- function(inf_prob, act_rate, pop_size = 1000, 
  ...
  incidence_rates <- as.data.frame(sim, out = "mean") %>% 
    ...
    mutate(act_rate = act_rate, inf_prob = inf_prob, 
